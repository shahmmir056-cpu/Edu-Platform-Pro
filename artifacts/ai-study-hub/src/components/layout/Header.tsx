...
export function Header() {
  ...
  const autoHideTimer = useRef<ReturnType<typeof setTimeout>>(null);
  const hideDelay = 3000;

  const isVirtualLab = location === "/virtual-lab";
  ...
  useEffect(() => {
    if (isHovering) {
      ...
      autoHideTimer.current = setTimeout(() => setHeaderVisible(false), hideDelay);
    }
    return () => clearTimeout(autoHideTimer.current!);
  }, [isHovering]);

  useEffect(() => {
    if (!isVirtualLab) setHeaderVisible(true);
  }, [isVirtualLab]);

  const isToolActive = TOOLS.some((t) => t.path === location);

  const handleHoverZoneEnter = useCallback(() => setIsHovering(true), []);

  const handleHoverZoneLeave = useCallback(() => setIsHovering(false), []);

  const handleHeaderMouseEnter = useCallback(() => setIsHovering(true), []);

  const handleHeaderMouseLeave = useCallback(() => setIsHovering(false), []);

  const showAutoHide = isVirtualLab && scrolled;
  ...
  return (
    <>
      {showAutoHide && (
        <div
          className={cn(
            "fixed top-0 left-0 right-0 z-[60] bg-transparent transition-opacity duration-300",
            headerVisible ? "pointer-events-none" : "pointer-events-auto"
          )}
          style={{ height: "4rem" }}
      ...
      <header
        ...
        className={cn(
          "sticky w-full transition-all duration-300",
          showAutoHide
            ? cn(
                "z-50",
                headerVisible
